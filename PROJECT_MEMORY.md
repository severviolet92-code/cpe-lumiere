...
>
> Last updated: 2026-07-19, after Phase 5 (complete-demo content build-out — see §5 "Phase 5"): nutrition/kitchen page, careers application form with CV upload (`job-applications`), group-scoped portal photo feed, staff departments, ~204-article knowledge base, full team/activities/gallery/docs seed expansion, `GESTION_DES_COMPTES.md`. Previous update: 2026-07-18, after Phase 3H (production-hardening / observability foundation — centralized error capture, structured logging, `/api/health` + admin System-health dashboard, boot-time env checks, fetch timeouts) plus `PRODUCTION_ACTIVATION_CHECKLIST.md`. Builds on Phase 3G (`seed:clear` rehearsal) and 3F (KB preview, working campaign scheduler, magic-link auth, verified Postgres swap, Law 25 PIA template). This document is the **architectural source of truth**, carried forward from the original planning sessions. Where the implementation has evolved beyond what was originally sketched, that evolution is documented explicitly *in place* — old plan and new reality side by side — rather than silently overwritten. Nothing in §2 (non-negotiable rules) has been weakened; where implementation details under a rule changed, the rule's *intent* was preserved and the *mechanism* was updated to match what actually ships.
>
...
**Phase 5 — Complete-demo content build-out (2026-07-19, local Windows session).** User request: make the demo feel 100 % complete for the director presentation. Everything below was implemented, seeded, verified (typecheck, lint, production build 0 errors, live browser tests, access-control matrix) in the user's local Windows environment (`c:\A-Projects\cpe-lumiere-demo\cpe-lumiere` — note the project moved from the path in §4):
- **Nutrition/kitchen section**: new `nutrition-page` global + public `/nutrition` page — intro, philosophy (richText), 5-day sample menu table (`weeklyMenu` array, localized), allergies richText, downloadable 4-week menu PDF (relationship to `documents`), meal-photo illustrations. Nav + footer + sitemap updated.
- **Careers application flow**: new `job-applications` upload collection (the CV *is* the upload; PDF/DOC/DOCX only; admin-only read/create/update/delete — Law 25 note in the admin description). Public form (`ApplicationForm.tsx` + server action `carrieres/actions.ts`) mirrors the contact-form pattern: honeypot, 3/30 min/IP rate limit, size (5 MB) + mimetype validation, published-opening validation, write via `overrideAccess: true`. `JobOpenings` gained a `qualifications` richText. Verified end-to-end in a real browser (a submission with an invalid PDF is rejected by Payload's file validation and surfaces the error message; a valid one lands in the admin — 3 docs incl. 2 seeded demos). CV upload dir `/job-applications` is gitignored (personal data).
- **Group-scoped portal photo feed**: `gallery-photos` gained `visibility` (`portal` **default** | `public` — same rule-3 reasoning as activities) and optional `groups` (hasMany). New `galleryPhotosRead` in `src/access/index.ts` (admin all; parent: public + untagged-portal + own-group-tagged; anonymous: public only — **verified live: anon 12, parent A 14, parent B 15**). New `/portail/photos` page + portal nav. **Gotcha fixed**: Next's image optimizer fetches upload files without the parent's cookie → portal photos 400'd; the portal page renders them `unoptimized` so the browser fetches directly with credentials.
- **Staff departments**: `staff-profiles.department` select (`administration`/`educators`/`kitchen`/`specialists`); Notre CPE renders the team in four titled sections. 16 fictional staff seeded with generated abstract "portrait" avatars (`renderPortrait()` in `illustrations.ts` — no people depicted, ever).
- **Notre CPE**: `about-page` gained `mission` (textarea) + `history` (richText), both seeded and rendered.
- **La vie au CPE**: `life-page` gained `groupsIntro`, `development`, `naps`, `outdoor`, `safety` (richText each); `groups` gained a localized `description`; the page now renders the five group cards + all sections + a CTA to /nutrition.
- **Knowledge base ×~18**: ~204 bilingual articles in `src/seed/kb/*.ts` (types + 5 content files + index), replacing the 13 inline ones. Same categories; realistic Québec-CPE content (La Place 0-5, contribution réduite, relevé 24, ratios, Gazelle et Potiron, Law 25…), CPE-specific claims labelled "(démo)".
- **Seed expansion**: 18 activities (near-term + seasonal: ferme, verger, citrouilles, carnaval d'hiver, kermesse…), 16 gallery photos (12 public / 4 portal incl. group-scoped), 4 job openings with qualifications, 2 demo job applications, 7 demo PDFs (multi-line `makeDemoPdf`), 11 closures (full year), 19 FAQ entries, 2 extra announcements, 16 new illustration themes. `seed:clear` list includes `job-applications`.
- **`GESTION_DES_COMPTES.md`** (new, repo root): the requested explanation of how accounts are created/activated/managed — director vs parent roles, parent-account lifecycle, demo credentials, non-negotiable security rules (FR with EN summary).
- Also fixed: a stray `P` character someone had typed into the Payload-generated `(payload)/admin/[[...segments]]/not-found.tsx` (broke typecheck).
...
**Status: Phase 5 (complete-demo content build-out) is done and verified on top of Phase 3 (A–H).** The demo now has a full nutrition section, a working careers application flow with CV upload, a group-scoped portal photo feed, a 16-person team in four departments, ~204 KB articles, and a year of seeded activities/closures/documents — the "100 % complete-feeling" presentation site the user asked for. **Environment note: this session ran on the user's own Windows machine, so `http://localhost:3000` IS directly reachable by the user** (unlike the cloud sessions described below). Phase 3 status for reference: code-complete and fully tested, merged into `main`. The application is NOT deployed anywhere, and per explicit user instruction must not be until every planned feature is implemented and verified AND the user has explicitly reviewed and approved going live.** Nothing is mid-flight. Everything buildable in code — including the production-hardening/observability layer — is done and verified. Phase 4 (real launch, see §8) remains genuinely not started: it requires real-world inputs (hosting account, domain, Postgres connection, email-provider + domain auth, legal sign-off) that only the user/director can provide, itemized in `PRODUCTION_ACTIVATION_CHECKLIST.md`.
...
**Phase 5 rows — all verified live (2026-07-19, local Windows dev):**

| Test | Expected result | Status |
|---|---|---|
| Anonymous GET `/api/gallery-photos` | Only `visibility:public` + published (12 of 16) | ✅ Pass |
| Parent A (Papillons) GET `/api/gallery-photos` | 14 (12 public + Papillons-tagged + untagged portal) | ✅ Pass |
| Parent B (Poussins+Explorateurs) GET `/api/gallery-photos` | 15 | ✅ Pass |
| Anonymous GET/POST `/api/job-applications` | `403` both | ✅ Pass |
| Careers form, valid PDF CV (real browser) | Success message; doc visible to director (3 total) | ✅ Pass |
| Careers form, malformed PDF | Rejected by Payload file validation; clean error message; captured by observability (`scope: careers`) | ✅ Pass |
| `/portail/photos` anonymous | `307` redirect to login | ✅ Pass |
| `/portail/photos` as parent A (real browser) | 14 photos, 0 broken images (after the `unoptimized` fix) | ✅ Pass |
| KB search "prix" anonymous | Relevant fee article, `gated:false` | ✅ Pass |
| KB search medication-dosage question | `gated:true`, fixed redirect | ✅ Pass |
| Every public page FR+EN (incl. new `/nutrition`) | `200` | ✅ Pass |
| Typecheck, ESLint, production build | 0 errors (36 routes incl. `/nutrition`, `/portail/photos`) | ✅ Pass |
